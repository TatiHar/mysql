...
CREATE TABLE `play_lists` (
	id SERIAL PRIMARY KEY,
	`name` VARCHAR(150) DEFAULT NULL COMMENT 'имя плейлиста',
	`user_id` BIGINT UNSIGNED DEFAULT NULL,
	FOREIGN KEY (user_id) REFERENCES users(id) ON UPDATE CASCADE ON DELETE CASCADE
);
...
CREATE TABLE `music`(
	id SERIAL PRIMARY KEY,
	name VARCHAR(150) DEFAULT NULL COMMENT 'название композиции',
...
CREATE TABLE video_collection (
	id SERIAL PRIMARY KEY,
	name VARCHAR(150) DEFAULT NULL COMMENT 'видео папка',
	user_id BIGINT UNSIGNED DEFAULT NULL,
	FOREIGN KEY (user_id) REFERENCES users(id) ON UPDATE CASCADE ON DELETE CASCADE
);
